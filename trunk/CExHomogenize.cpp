...
#include "CExHomogenize.h"

CExHomogenize::CExHomogenize() {
...
void CExHomogenize::Execute()
{
	...
	Gauss_Coord(&inMesh, gCoord);
	    ...
	    // FD check for diff_E
	    //inMesh.mat_type[0]=1;
	    //inMat[1].rho = 9.50E-07;
	    //inMat[1].mat[0] = 1.048901099;
	...
	for(i=0;i<numMat;i++)
	{
		...
		KEMatrix2(KE[i], &inMat[i], inMesh.t);	// Element Stiffness Matrix for an IN element
	    ...
	    KEMatrix(KE1, &inMat[0], inMesh.t);	// Element Stiffness Matrix for an IN element
	    KEMatrix2(KE2, &inMat[0], inMesh.t);	// Element Stiffness Matrix for an IN element
	...
	int itt0 = 0;
	int rewind_cnt = 0; // count number of rewinds for current iteration
	double refact = 0.5; // rewind factor
	double *lsf_old = malloc(levelset.num * sizeof(double)); // store old lsf for rewind
	...
	// homogenization data
	//int *fixed_hom[2]; // 2 BCs required for homogenization
	sp_mat hom_ce; hom_ce.A=0; hom_ce.irn=0; hom_ce.jcn=0;
	isoMat homMat;
	//if(lsprob.obj>100){hom_bc(&inMesh, fixed_hom, &freeDof); numCase =3; }
	if(lsprob.obj>100){hom_bc2(&inMesh, &hom_ce); numCase =3; }
	...
	// compliance, 1st eigenvalue, mechanism objective or homogenization
	if(lsprob.obj==1 || lsprob.obj==3 || lsprob.obj==5 || lsprob.obj>100){ num_sens++; }
	for(i=0;i<lsprob.num;i++)
	{
		j = lsprob.con[i].type;
		if(j==3 || j==5 || j==7 || j==8 || j==101){ num_sens++; } // compliance or displacement constraint
	...
	else if(lsprob.obj > 100){
	    prim_ptr = malloc(9 * sizeof(double*));
	    dual_ptr = malloc(9 * sizeof(double*));
	    wgt = calloc(9, sizeof(double)); // 9 entries in homogenized tensor
	}
	...
	do {
	    ...
	    if(lsprob.obj < 100){rem_sp_mat(&Kg, fixDof, 1);}
		...
		if(lsprob.obj != 3)
		{
	        ...
	        if(lsprob.obj>100)
	        {
	            //homogenize(&inMesh, inMat, &Kg, alpha, fixed_hom, freeDof, &homMat, disp);
	            homogenize2(&inMesh, inMat, &Kg, alpha, &fixDof, freeDof, &hom_ce, &homMat, disp, control.pinfo);
	            if(lsprob.obj == 101)
	            {
	                obj[itt] = homMat.e;
	                printf("\nYoung's modulus = %12.4e",homMat.e);

	                // modify weight values
	                ftemp = homMat.mat[1] / homMat.mat[4]; // E12 / E22
	                wgt[0] = 1.0;
	                wgt[1] = -ftemp;
	                wgt[3] = -ftemp;
	                wgt[4] = ftemp * ftemp;
	            }
	            else if(lsprob.obj == 102)
	            {
	                // proper weighting for bulk modulus opt
	                wgt[0]=0.25; wgt[1]=0.25; wgt[3]=0.25; wgt[4]=0.25;
	                obj[itt] = homMat.k;
	                printf("\nBulk modulus = %12.4e",homMat.k);
	            }
	            else if(lsprob.obj == 103)
	            {
	                wgt[8] = 1.0;
	                obj[itt] = homMat.g;
	                printf("\nShear modulus = %12.4e",homMat.g);
	            }
	            //for(i=0;i<9;i++){printf("\nwgt[%i]=%f",i,wgt[i]);}

	            for(i=0;i<lsprob.num;i++){
	                if(lsprob.con[i].type==101){
	                    j = lsprob.num * itt + i; // point to place in cnstr
	                    ftemp = homMat.mat[0] - homMat.mat[4];
	                    cnstr[j] = ftemp; // store error value
	                    printf("\nSquare symm error = %12.4e",cnstr[j]);
	                }
	            }
	        }
	        // Normal static solve
		...
		if(lsprob.obj==1){
	        ...
	        sens_ptr[0] = &Nsens[temp++];
	        k=0; // count
	        for(i=0;i<3;i++)
	        {
	            for(j=0;j<3;j++)
	            {
	                prim_ptr[k] = &disp[i*order];
	                dual_ptr[k] = (fabs(wgt[k])<1.0e-9) ? NULL : &disp[j*order];
	                k++;
	            }
	        }
	    }
	    /*else if(lsprob.obj==101 || lsprob.obj==102) {
	        sens_ptr[0] = &Nsens[temp++]; // first array of Nsens for objective

	        prim_ptr[0] = disp; // 1st disp vector
	        prim_ptr[1] = &disp[order]; // 2nd disp vector
	        prim_ptr[2] = prim_ptr[0];

	        dual_ptr[0] = prim_ptr[0];
	        dual_ptr[1] = prim_ptr[1];
	        dual_ptr[2] = prim_ptr[1];
	    }
	    else if(lsprob.obj==103) {
	        sens_ptr[0] = &Nsens[temp++]; // first array of Nsens for objective
	        prim_ptr[0] = &disp[2*order]; // 3rd disp vector
	        dual_ptr[0] = prim_ptr[0];
	    }*/
	    ...
	    {
	        // compute boundary shape sensitivies (for homogenization)
	        //temp = (lsprob.obj==103) ? 1 : numCase;
	        AFG_Sens(&inMesh, &Bndry, alpha, inMat, Nsens, prim_ptr, dual_ptr, 1, 9, wgt, gCoord, control.aMin, 0,fact,sw,acc);

	        // for square symmetry
	        for(i=0;i<lsprob.num;i++){
	            if(lsprob.con[i].type==101){
	                prim_ptr[0] = disp; prim_ptr[1] = &disp[order];
	                dual_ptr[0] = prim_ptr[0]; dual_ptr[1] = prim_ptr[1];
	                ftemp = homMat.mat[0] - homMat.mat[4];
	                oMax = AreaElem * (double)(inMesh.NumElem);
	                oMax = 1.0 / oMax;
	                double wgt2[2];
	                if(ftemp > 0.0){wgt2[0] = -oMax; wgt2[1] = oMax;}
	                else{wgt2[0] = oMax; wgt2[1] = -oMax;}
	                AFG_Sens(&inMesh, &Bndry, alpha, inMat, &Nsens[Ntot], prim_ptr, dual_ptr, 1, 2, wgt2, gCoord, control.aMin, 0,fact,sw,acc);
	            }
	        }
	        free(disp);
	    }
		...
		{
	        // check
	        //int tnodes[4]; tnodes[0]=0; tnodes[1]=1; tnodes[2]=2; tnodes[3]=3;
	        //double hom_wgt[10];
	        //diff_E(tnodes, prim_ptr, dual_ptr, alpha[0], inMesh.h, inMesh.t, hom_wgt, numCase, num_sens, wgt, sw, acc);
		...
		for(i=0;i<j;i++)
		{
	        ...
	        else if(lsprob.con[i].type==101)
	        {
	            delcon[i] = lsprob.con[i].data[0] - ftemp;
	            printf("\nSquare symm constraint %i violated by %12.4e\n",i+1,delcon[i]);
	            if(fabs(delcon[i]/homMat.mat[0]) > 1.0e-3){temp=0;}
	            delcon[i] = -fabs(delcon[i]);
	        }
		}
		...
		j = inMesh.NumNodes;
	    cblas_dcopy(j, levelset.lsf, 1, lsf_old, 1); // copy current lsf -> old (for rewind)
		for(k=0;k<j;k++)
		{
	    ...
	    {
	        rewind_cnt++;
	        printf("\nRewind no %i",rewind_cnt);

	        ftemp = 1.0-refact;
	        j = inMesh.NumNodes;
	        for(k=0;k<j;k++)
	        {
	            levelset.lsf[k] *=  ftemp;
	            levelset.lsf[k] +=  refact * lsf_old[k]; // linear rewind step
	        }
	    }

	    // moved from above if statement
	    free(Nsens);
