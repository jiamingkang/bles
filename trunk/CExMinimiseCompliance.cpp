...
int CExMinimiseCompliance::InitialiseOOP(char *path)
{
	int res = -1;
	...
	for (i = 0; i < numMat; i++)
	{
		...
		m_fem.KEMatrix(KE[i], &inMat[i], m_mesh.m_thinkness);	// Element Stiffness Matrix for an IN element
		m_fem.MEMatrix(ME[i], inMat[i].rho, AreaElem, m_mesh.m_thinkness); // Element Mass Matrix for an IN element
	}
	...
	sens_ptr = (double **) malloc( (1+lsprob.num) * sizeof(double *) ); // pointer to sensitivity arrays for each fucntion (obj + constraints)
	...
	Bndry.AuxNodes = (Coord *) malloc(m_mesh.m_numElem * sizeof(Coord));
	Bndry.Bound = (Bseg *) malloc(m_mesh.m_numElem * sizeof(Bseg));
	...
	Bndry.na_conn_ind = (int *) malloc( (m_mesh.m_numNodes+1) * sizeof(int)); // memory allocation
	lambda = (double *) calloc(1+lsprob.num, sizeof(double)); // lambda values
	...
	obj = (double *) malloc(control.maxItt*sizeof(double));
	cnstr = (double *) malloc(lsprob.num*control.maxItt*sizeof(double));
	pred = (double *) malloc((1+lsprob.num)*control.maxItt*sizeof(double)); // predicted obj & constraint change
	pred_temp = (double *) malloc((1+lsprob.num)*sizeof(double)); // temp to feed SLPsubSol
	delcon = (double *) malloc(lsprob.num*sizeof(double)); // array to store distance from constraint (constraint violation)
	relax = (double *) malloc(lsprob.num*sizeof(double)); // relaxation factor
	...
	CHakMaterial cmat;
	CHakSolver csolver;
	CHakSensitivity csensitivity;
	CHakBoundary cboundary;
...
int CExMinimiseCompliance::AnalyseOOP(int itt)
{
	...
	//do {
		...
		m_mesh.Find_struct(&inMesh, &levelset, &Bndry, alpha, control.aMin);
		Ntot = m_mesh.m_numNodes + Bndry.NumAux; // total number of nodes

		if(control.pinfo > 1 || itt==0)	{
			// output lsf (& alpha) values
			m_output.OutPLotShapeVTK2(m_mesh, levelset.lsf, alpha, control.pinfo, itt, filename);
		}
		...
		for(i=0;i<lsprob.num;i++)
		{
			...
			{
				ftemp = 0.0;
				if(m_mesh.des_mat){temp=0; oMin = inMat[m_mesh.mat2].rho / inMat[m_mesh.mat1].rho;}
				for(j=0;j<m_mesh.m_numElem;j++) {
					oMax = alpha[j] * inMat[m_mesh.mat_type[j]].rho * m_mesh.m_thinkness; // area ratio x density
...
int CExMinimiseCompliance::SensitivityOOP(void)
{
		...
		for(i=0;i<lsprob.num;i++)
		{
			...
			else if(j==2)
			{
				...
				if(m_mesh.des_mat)
				{
					...
					for(k=0;k<m_mesh.m_numElem;k++)
					{
						if(k==Bndry.Bound[j2].e)
						{
							...
							//ftemp = ftemp*inMat[m_mesh.mat1].rho + (1.0-ftemp)*inMat[m_mesh.mat2].rho;
	                        ftemp = (1.0-ftemp)*inMat[m_mesh.mat1].rho + ftemp*inMat[m_mesh.mat2].rho;
				...
				{
					ftemp = -inMat[0].rho * m_mesh.m_thinkness; // mass senstivity (assumed) density x thickness
		...
		if(m_mesh.bars && lsprob.obj == 1)
		{
			...
			// output bar info
			m_output.OutBars(m_mesh, (lsprob.num+1), bar_sens, control.pinfo, itt, filename);
		}

		// additional bc sensitivities
		if(m_mesh.des_bc && lsprob.obj == 1)
		{
			...
			m_output.OutDesBC(m_mesh, bc_sens, control.pinfo, itt, filename);
		}
		...
		if(m_mesh.des_mat)
		{
	        ...
	        if(lsprob.obj == 1)
	        {
	            ...
	            if(m_mesh.mat_lin){ csensitivity.matSens_comp(&inMesh, inMat, KE[0], mat_sens_temp, numCase, wgt, prim_ptr, dual_ptr, alpha, control.aMin, sw, acc); }
	            //else{ HS_Sens_eig(&m_mesh, inMat, KE[0], ME[0], mat_sens_temp, num_eig, eig_vals, prim_ptr, alpha, control.aMin); }
	        ...
	        if(lsprob.obj == 3)
	        {
	            ...
	            if(m_mesh.mat_lin){ csensitivity.matSens_eig(&inMesh, inMat, KE[0], ME[0], mat_sens_temp, num_eig, eig_vals, prim_ptr, alpha, control.aMin); }
	        ...
	        for(i=0;i<lsprob.num;i++)
	        {
	            ...
	            else if(lsprob.con[i].type==2)
	            {
	                ...
	                ftemp = (inMat[m_mesh.mat2].rho - inMat[m_mesh.mat1].rho) * m_mesh.m_lenEdge * m_mesh.m_lenEdge * m_mesh.m_thinkness;
	                k = m_mesh.NumDesMat * (i+1);
	                for(j=0;j<m_mesh.NumDesMat;j++) {
			...
			m_output.OutDesMat(m_mesh, alpha, control.aMin, (lsprob.num+1), mat_sens, control.pinfo, itt, filename);
		}
		...
		if(control.pinfo == 3)	{
			...
			m_output.OutBoundVTK(m_mesh, &Bndry, (lsprob.num+1), sens_ptr, itt, filename);
		}
...
int CExMinimiseCompliance::OptimiseOOP(void)
{
		...
		if(control.pinfo == 3)	{
			...
			m_output.OutBoundInt((lsprob.num+1), num_Lbound, Lbound_nums, Lbound, itt, filename);
		}
		...
		if(m_mesh.bars)
		{
			temp = m_levelset.SLPsubSol4(&inMesh, &levelset, &Bndry, cfl, delcon, lsprob.num, sens_ptr, Lbound, num_Lbound, Lbound_nums, lambda, active, Vnorm, pred_temp, m_mesh.NumBars, bar_sens, bar_min, bar_max, bar_step, control.pinfo);
		}
		else if(m_mesh.des_bc)
		{
			// solve for level set velocity
			temp = m_levelset.SLPsubSol4(&inMesh, &levelset, &Bndry, cfl, delcon, lsprob.num, sens_ptr, Lbound, num_Lbound, Lbound_nums, lambda, active, Vnorm, pred_temp, 0, 0, 0, 0, 0, control.pinfo);
			...
			csolver.LPsolve(m_mesh.NumBC, 1, m_mesh.NumBC, bc_step, bc_sens, &bc_sens[m_mesh.NumBC], &ftemp, bc_max, control.pinfo);
		...
		else if(m_mesh.des_mat)
		{
	        if(m_mesh.dm_sim || mat_opt)
	        {
	            ...
	            //temp = SLPsubSol4(&inMesh, cfl, delcon, lsprob.num, sens_ptr, Lbound, num_Lbound, Lbound_nums, lambda, active, Bndry.AuxNodes, Vnorm, pred_temp, m_mesh.NumDesMat, mat_sens, mat_min, mat_max, mat_step, control.pinfo);

	            if(m_mesh.dm_sim)
	            {
	                // solve for simultaneous update
	                temp = m_levelset.SLPsubSol4(&inMesh, &levelset, &Bndry, cfl, delcon, lsprob.num, sens_ptr, Lbound, num_Lbound, Lbound_nums, lambda, active, Vnorm, pred_temp, num_reduced, mat_sens_red, mat_min, mat_max, mat_step_red, control.pinfo);
		...
		m_levelset.Vext(&inMesh, &levelset, &Bndry, Vnorm);
			...
			for(i=1;i<i2;i++)
			{
				...
				if(levelset.active[k]) // If node is active (not fixed and within narrow band)
				{
					if(fabs(Vnorm[k]) > 1.0e-6) // If normal velocity not zero
					{
						...
						Grad[k] = m_levelset.GradWENO(i, j, k, levelset.lsf, &inMesh, temp, Vnorm[k], 1.0);
					}
				}
			}
		}
		...
		if(ReCount < 19)
		{
			for(j=0;j<levelset.numMine;j++) // for all mine nodes
			{
				...
				if( (fabs(levelset.lsf[levelset.mine[j]]) -  m_mesh.m_lenEdge) < m_mesh.m_tolerance )
				{
					temp = 1;
		...
		if(temp == 1)
		{
			...
			m_levelset.ReInt(&inMesh, &levelset); // reinitialize the lsf
			m_levelset.NarBand(&inMesh, &levelset, control.lband); // re-calculate the bandwidth
...
void CExMinimiseCompliance::OutputOOP(void)
{
	...
	if(control.pinfo == 1) {
		m_output.OutPLotShapeVTK2(m_mesh, levelset.lsf, alpha, 1, itt, filename);
	}

	// Write Objective and Constraint convergence info file
	if(itt == control.maxItt){ itt--; }
	m_output.OutConv(itt, &lsprob, obj, cnstr, filename);
...
int CExMinimiseCompliance::Initialise(char *path)
{
    ...
    if(control.pinfo==3)
    {
        m_output.OutNumber(&inMesh, filename);
        m_output.OutNodeCoord(&inMesh, filename);
    }
    ...
    // designable material varibles
    mat_opt = inMesh.dm_sim; // flag to switch between seq & sim opt
...
int CExMinimiseCompliance::Analyse(int itt)
{
	...
	//do {
		...
		if(control.pinfo > 1 || itt==0)	{
			// output lsf (& alpha) values
			m_output.OutPLotShapeVTK2(&inMesh, levelset.lsf, alpha, control.pinfo, itt, filename);
		}
		...
		if(control.pinfo == 3) {
			// write displacement & mode shape info file
			temp = (lsprob.obj==3) ? 0 : numCase;
			m_output.OutDispVTK(&inMesh, temp, disp, 2*num_eig, eig_vecs, itt, filename);
		}
}


int CExMinimiseCompliance::Sensitivity(void)
{
		...
		if(inMesh.bars && lsprob.obj == 1)
		{
			...
			// output bar info
			m_output.OutBars(&inMesh, (lsprob.num+1), bar_sens, control.pinfo, itt, filename);
		}
		...
		if(inMesh.des_bc && lsprob.obj == 1)
		{
			...
			// output designable BC info
			m_output.OutDesBC(&inMesh, bc_sens, control.pinfo, itt, filename);
		}
		...
		if(inMesh.des_mat)
		{
			...
			// output material info
			m_output.OutDesMat(&inMesh, alpha, control.aMin, (lsprob.num+1), mat_sens, control.pinfo, itt, filename);
		}
		...
		if(control.pinfo == 3)	{
			// Write node sensitvity information file (as a boundary mesh)
			m_output.OutBoundVTK(&inMesh, &Bndry, (lsprob.num+1), sens_ptr, itt, filename);
		}
...
int CExMinimiseCompliance::Optimise(void)
{
		...
		if(inMesh.bars)
		{
			temp = m_levelset.SLPsubSol4(&inMesh, &levelset, &Bndry, cfl, delcon, lsprob.num, sens_ptr, Lbound, num_Lbound, Lbound_nums, lambda, active, Vnorm, pred_temp, inMesh.NumBars, bar_sens, bar_min, bar_max, bar_step, control.pinfo);
		}
		...
		if(control.pinfo == 3)	{
			// Write Node Gradient & Velocity information file
			m_output.OutHJVTK(&inMesh, Vnorm, Grad, itt, filename);
		}
		free(Grad);
		free(Vnorm);
...
void CExMinimiseCompliance::Output(void)
{
	if(itt == control.maxItt) {
		printf("\nSolution stopped after %i iterations\nObjective Value = %12.4e\n",itt,obj[itt-1]);
	}
	...
	// Output the final ls function
	if(control.pinfo == 1) {
		m_output.OutPLotShapeVTK2(&inMesh, levelset.lsf, alpha, 1, itt, filename);
	}
